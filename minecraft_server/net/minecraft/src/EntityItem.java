...
public class EntityItem extends Entity
{
	...
	private void CheckForItemDespawn() {
		if (!worldObj.isRemote) {
			if (droppingPlayerDeathsOnDrop > -1) {
				...
				// Legacy item despawn timer, left in for items in world that were still using this
	...
	public void SetEntityItemAsDroppedOnPlayerDeath(EntityPlayer player) {
		if (!worldObj.isRemote) {
			...
			// subtract time from grace time if dying within 9minutes (TODO possibly only works for HC spawn, but that's okay
