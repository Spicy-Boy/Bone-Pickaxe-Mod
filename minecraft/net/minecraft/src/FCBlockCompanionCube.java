...
public class FCBlockCompanionCube extends Block
{
	...
	public boolean canToolStickInBlockSpecialCase(World world, int x, int y, int z, Item toolOrSword) {
		if (toolOrSword instanceof FCItemSword) {
			if (!world.isRemote) {
				if (!GetIsSlab(world, x, y, z)) {
					world.playSoundEffect((float) x + 0.5F, (float) y + 0.5F, (float) z + 0.5F, "mob.wolf.whine", 0.5F,
    ...
    public void registerIcons( IconRegister register )
    {
        ...
        m_IconFront = register.registerIcon( "fcBlockCompanionCube_front" );
        m_IconGuts = register.registerIcon( "fcBlockCompanionCube_guts" );
    }
